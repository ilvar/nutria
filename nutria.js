...
Categories = new Mongo.Collection("cat_words");

if (Meteor.isClient) {
    Template.navmenu.helpers({
        section_products: function () {
            return Session.get("section") != 'categories';
        }
    });
    ...
    Template.products.events({
        'keyup form.search input': function (e, template) {
            Session.set("search", event.target.value);
        }
    });

    Template.navmenu.events({
        'click #menu_products': function () {
            return Session.set("section", 'products');
        },
        'click #menu_cats': function () {
            return Session.set("section", 'categories');
        }
    });

    Template.categories.events({
        'click .select-cat': function (e, template) {
            var c = Categories.findOne({Category: parseInt(e.target.dataset.cat)});
            console.log(c.Words);
            var max_weight = _.max(_.map(c.Words, function (w) { return w[1] }));
            var max_length = _.max(_.map(c.Words, function (w) { return w[0].length }));
            var add_scale = Math.floor(max_length / 7);
            if (add_scale) {
                max_weight = max_weight / add_scale;
            }
            WordCloud(document.getElementById('words_cloud'), {
                clearCanvas: true,
                backgroundColor: "white",
                fontFamily: "Finger Paint, cursive, sans-serif",
                gridSize: Math.round(24.0 * 848 / 1024),
                weightFactor: 108.0 / max_weight,
                rotateRatio: 0,
                minSize: 12,
                list: c.Words,
